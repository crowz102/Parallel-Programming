...
class AlgorithmMapper extends MapReduceBase implements Mapper<Text, Text, Text, Text>
{
    ...
    private static final String UNMARKED               = "UNMARKED";
    ...
    public void map(Text key, Text value, OutputCollector<Text, Text> outputMapper, Reporter reporter) throws IOException
    {
        ...
        if (sourceNodeDetails.length >= MINIMUM_SOURCE_NODE_TOKENS
                ...
                && !sourceNodeDetails[INDEX_SOURCE_NODE_STATUS].equalsIgnoreCase(MARKED))
        {
            String currentPath = "";
            ...
            outputMapper.collect(new Text("{" + sourceNodeDetails[INDEX_SOURCE_NODE_NUMBER] + "," + sourceNodeDetails[INDEX_SOURCE_NODE_WEIGHT] + "," + MARKED + "," + currentPath + "}"), value);
            System.out.println(">>> MAP1: Key = [" + new Text("{" + sourceNodeDetails[INDEX_SOURCE_NODE_NUMBER] + "," + sourceNodeDetails[INDEX_SOURCE_NODE_WEIGHT] + "," + MARKED + "," + currentPath + "}") + "] Value = [" + value +"]");
            ...
            if (value.toString().trim().length() > 0)
            {
                ...
                for (int index = 0; index < tokens.length; index++)
                {
                    ...
                    outputMapper.collect(new Text("{" + adjacentNodeDetails[index][0] + "," + Integer.toString(number) + "," + UNMARKED + "," + currentPath + "}"), emptyText);

                    reporter.incrCounter(ShortestPath.CUSTOM_COUNTERS, ShortestPath.NUMBER_OF_UNMARKED_NODES_TO_BE_PROCESSED, 1);
                    System.out.println(">>> MAP2: Key = [" + new Text("{" + adjacentNodeDetails[index][0] + "," + Integer.toString(number) + "," + UNMARKED + "," + currentPath + "}") + "] Value = [" + emptyText +"]");
                }
            }
        }
        else
...
class AlgorithmPartitioner implements Partitioner<Text, Text>
{
    ...
    public int getPartition(Text key, Text value, int numReduceTasks)
    {
        ...
        int returnValue = (nodeDetails[AlgorithmMapper.INDEX_SOURCE_NODE_NUMBER].hashCode() & Integer.MAX_VALUE)% numReduceTasks;
...
public class ShortestPath extends Configured implements Tool
{
    ...
    public static final String NUMBER_OF_UNMARKED_NODES_TO_BE_PROCESSED = "NUMBER_OF_UNMARKED_NODES_TO_BE_PROCESSED";
    ...
    public int run(String[] args) throws Exception
    {
        ...
        do
        {
            ...
            iteration++;
            toBeProcessed = job.getCounters().findCounter(CUSTOM_COUNTERS, NUMBER_OF_UNMARKED_NODES_TO_BE_PROCESSED).getValue();
